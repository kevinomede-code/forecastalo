...
export const Route = createFileRoute("/screening")({
  head: () => ({
    meta: [
      { title: "Screening — Housing & Energy Investment Scores in Italy" },
      {
        name: "description",
        content:
          "Screen Italian provinces, municipalities and market zones for housing and energy investment opportunities with forecast-driven scores.",
      },
      {
        property: "og:title",
        content: "Forecastalo Screening — Investment Scores for Italy",
      },
      {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background text-foreground">
      ...
      <main className="flex min-h-0 flex-1 gap-5 overflow-hidden p-5">
        {/* The whole left column scrolls as one continuous area. */}
        <section className="flex w-[40%] flex-col gap-5 overflow-y-auto pr-1">
          ...
          <div className="rounded-2xl border border-border bg-panel shadow-soft">
            ...
            <div className="p-4">
              {request && !query.isFetching && !query.isError && results.length === 0 ? (
                <div className="mx-auto my-8 max-w-xs text-center">
                  <p className="text-sm font-medium">No scores computed yet</p>
                  <p className="mt-2 text-sm text-muted-foreground">
                    The database is set up, but no zones have been scored so far. Results will
